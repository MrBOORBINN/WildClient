        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-lg glass-card p-6 rounded-[24px] border border-white/10">
          ...
          <div className="space-y-4">
            <textarea 
              ...
              className="w-full h-24 bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:ring-1 focus:ring-[#4285f4] outline-none text-white"
            />
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#8e918f] uppercase">Style</label>
                <select value={style} onChange={(e) => setStyle(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-xs outline-none text-white">
              ...
              <div className="space-y-1.5">
                ...
                <select value={aspectRatio} onChange={(e) => setAspectRatio(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-xs outline-none text-white">
                  {['1:1', '16:9', '9:16'].map(a => <option key={`photo-ratio-${a}`} value={a} className="bg-[#1e1f20]">{a}</option>)}
                </select>
              </div>
            </div>
            <button onClick={onGenerate} className="w-full py-3 bg-gradient-to-r from-[#4285f4] to-[#9b72cb] rounded-xl font-bold text-sm shadow-lg hover:scale-[1.02] transition-transform mt-2 text-white">Generate Image</button>
          </div>
        </motion.div>
      </div>
    )}
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-lg glass-card p-6 rounded-[24px] border border-white/10">
          ...
          <div className="space-y-4">
            <textarea 
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Describe the audio track you want to compose..."
              className="w-full h-24 bg-white/5 border border-white/10 rounded-xl p-3 text-sm focus:ring-1 focus:ring-[#34a853] outline-none text-white"
            />
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#8e918f] uppercase">Genre</label>
                <select value={genre} onChange={(e) => setGenre(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-xs outline-none text-white">
              ...
              <div className="space-y-1.5">
                ...
                <select value={mood} onChange={(e) => setMood(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-xs outline-none text-white">
            ...
            <button onClick={onGenerate} className="w-full py-3 bg-gradient-to-r from-[#34a853] to-[#4285f4] rounded-xl font-bold text-sm shadow-lg hover:scale-[1.02] transition-transform mt-2 text-white">Compose Track</button>
          </div>
        </motion.div>
      </div>
    )}
          ...
          {!isConnected ? (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4 py-12">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-[#8e918f]">Access and analyze your documents directly in INFBOTT</p>
              </div>
              <button 
                onClick={onConnect}
                className="px-6 py-2 bg-[#4285f4] hover:bg-[#4285f4]/80 rounded-full font-bold transition-colors text-white"
              >
                Connect Now
              </button>
            </div>
...
export const HelpModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  const faqs = [
    {
      question: "What is INFBOTT?",
      answer: "INFBOTT is an advanced, production-ready AI assistant designed for research, coding, game development, multi-file analysis, and productivity. It natively understands English, Hindi, and Hinglish with high speed and precision."
    },
    {
      question: "How does Web Research work?",
      answer: "INFBOTT integrates Google Search Grounding to fetch verified, up-to-date information across the live web and synthesizes clean responses with source citations."
    },
    {
      question: "Which files can I upload?",
      answer: "You can upload Images (PNG, JPG, WEBP, GIF), PDF documents, Word DOCX files, CSV spreadsheets, JSON schemas, Markdown, and code files (JS, TS, Python, HTML, CSS, SQL, Bash)."
    },
    {
      question: "Can INFBOTT build and play games?",
      answer: "Yes! INFBOTT can write complete games in HTML5 Canvas and JavaScript. You can run and play the code directly in the Interactive Code & Game Runner sandbox."
    },
    {
      question: "How do I export my chat as PDF?",
      answer: "Click the 'Export PDF' button in the chat header to download the full conversation history as a styled, paginated PDF document."
    }
  ];
          ...
          >
            <div className="p-8 border-b border-white/5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold text-white">INFBOTT System & FAQ</h2>
              </div>
              <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors text-[#8e918f] hover:text-white">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              {faqs.map((faq, index) => (
                <div key={`modal-faq-${index}`} className="space-y-2">
                  ...
                  <p className="text-[#8e918f] leading-relaxed pl-10 text-sm">
                    {faq.answer}
                  </p>
                </div>
              ))}
          ...
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-2xl glass-card p-8 rounded-[32px] border border-white/10">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-bold flex items-center gap-3 text-white">
                ...
                Appearance & Canvas Settings
              </h2>
              <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full text-[#8e918f] hover:text-white"><X size={24} /></button>
            </div>
            ...
            <div className="space-y-8">
              <div>
                ...
                <button 
                  ...
                  className="w-full py-8 border-2 border-dashed border-white/10 rounded-2xl hover:bg-white/5 transition-all flex flex-col items-center gap-2 text-[#8e918f] hover:text-white cursor-pointer"
                >
                  <ImageIcon size={32} />
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <button 
                    ...
                    className={cn(
                      "aspect-video rounded-xl border-2 transition-all flex items-center justify-center bg-[#131314] cursor-pointer",
                      currentBackground === null ? "border-[#ff4e00]" : "border-transparent hover:border-white/20"
                    )}
                  >
                    <span className="text-xs font-bold text-white">Default Liquid</span>
                  </button>
                  {gallery.map((bg, bIdx) => (
                    <button 
                      ...
                      className={cn(
                        "aspect-video rounded-xl border-2 transition-all overflow-hidden relative group cursor-pointer",
